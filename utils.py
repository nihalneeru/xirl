...
def wrap_learned_reward(env, config):
  ...
  elif config.reward_wrapper.type == "cosine_similarity":
    kwargs["goal_emb"] = load_pickle(pretrained_path, "goal_emb.pkl")
    env = wrappers.CustomLearnedVisualReward(**kwargs)

  elif config.reward_wrapper.type == "temporal_difference":
    kwargs["time_penalty"] = config.reward_wrapper.get("time_penalty", 0.1)
    env = wrappers.TemporalDifferenceLearnedVisualReward(**kwargs)

  elif config.reward_wrapper.type == "collision_penalty":
    kwargs["collision_threshold"] = config.reward_wrapper.get("collision_threshold", 0.5)
    kwargs["penalty"] = config.reward_wrapper.get("penalty", -1.0)
    env = wrappers.CollisionPenaltyLearnedVisualReward(**kwargs)

  elif config.reward_wrapper.type == "path_smoothness":
    kwargs["smoothness_penalty"] = config.reward_wrapper.get("smoothness_penalty", 0.1)
    env = wrappers.PathSmoothnessLearnedVisualReward(**kwargs)

  else:
    raise ValueError(f"Unsupported reward wrapper type: {config.reward_wrapper.type}")
