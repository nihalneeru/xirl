...
class CustomLearnedVisualReward(LearnedVisualReward):
    """Custom reward function that computes cosine similarity to a goal embedding."""

    def __init__(self, env, model, device, res_hw, goal_emb):
        """
        Constructor.

        Args:
            env: gym env
            model: A model that extracts embeddings from observations
            device: Device for computation ('cpu' or 'cuda')
            res_hw: Optional (H, W) to resize the environment image before feeding it to the model
            goal_emb: Goal embedding to compare against for cosine similarity
        """
        super().__init__(env, model, device, res_hw)
        self.goal_emb = torch.tensor(goal_emb, device=self._device).unsqueeze(0)

    def _get_reward_from_image(self, image):
        image_tensor = self._to_tensor(image)
        current_emb = self._model.infer(image_tensor).embs
        current_emb = torch.tensor(current_emb, device=self._device)
        cosine_similarity = torch.nn.functional.cosine_similarity(current_emb, self.goal_emb, dim=1)
    ...
    def step(self, action):
        ...
        pixels = self._render_obs()
        learned_reward = self._get_reward_from_image(pixels)
        info["env_reward"] = info.get("reward", 0.0)
        info["learned_reward"] = learned_reward
        return obs, learned_reward, done, info


class TemporalDifferenceLearnedVisualReward(gym.Wrapper):
    """Custom reward function based on temporal difference (time penalty)"""

    def __init__(self, env, time_penalty=0.1):
        """
        Args:
            env: gym env
            time_penalty: Penalty for each step taken without reaching the goal
        """
        super().__init__(env)
    ...
    def step(self, action):
        ...
        temporal_reward = -self.time_penalty * self.step_count
        info["temporal_penalty"] = temporal_reward

        return obs, temporal_reward, done, info
...
class CollisionPenaltyLearnedVisualReward(gym.Wrapper):
    """Custom reward function that penalizes collisions"""

    def __init__(self, env, collision_threshold=0.5, penalty=-1.0):
        """
        Args:
            env: gym env
            collision_threshold: Distance threshold for collision detection
            penalty: Penalty value when a collision is detected
        """
        super().__init__(env)
    ...
    def _check_collision(self, obs):
        position = obs[:2]
        distance_to_obstacle = np.linalg.norm(position)
        return distance_to_obstacle <= self.collision_threshold

    def step(self, action):
        obs, reward, done, info = self.env.step(action)
        collision = self._check_collision(obs)
        if collision:
            reward += self.penalty
            info["collision"] = True
        else:
...
class PathSmoothnessLearnedVisualReward(gym.Wrapper):
    """Custom reward function that encourages smooth paths"""

    def __init__(self, env, smoothness_penalty=0.1):
        """
        Args:
            env: gym env
            smoothness_penalty: Penalty for sharp changes in direction
        """
        super().__init__(env)
    ...
    def step(self, action):
        ...
        if self.previous_action is not None:
            # smoothness penalty = difference between consecutive actions
            action_diff = np.linalg.norm(np.array(action) - np.array(self.previous_action))
            smoothness_reward = -self.smoothness_penalty * action_diff
            reward += smoothness_reward
            info["smoothness_penalty"] = smoothness_reward
